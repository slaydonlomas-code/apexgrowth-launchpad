...
function Panel({
  ...
  return (
    <div className={`h-full rounded-2xl border bg-card p-7 ${accent ? "border-primary/30 shadow-elegant" : "border-border"}`}>
      <div className="flex items-center gap-3">
        <span
          className={`grid h-11 w-11 place-items-center rounded-xl ${
            accent ? "bg-gold-gradient text-primary-foreground shadow-gold" : "border border-border bg-muted text-foreground"
          }`}
        >
      ...
      <ul className="mt-6 space-y-3 text-sm text-foreground">
        {items.map((i) => (
          <li key={i} className="flex gap-3">
            <span className={`mt-2 h-1.5 w-1.5 shrink-0 rounded-full ${accent ? "bg-green" : "bg-primary/50"}`} aria-hidden="true" />
            {i}
          </li>
        ))}
      </ul>
