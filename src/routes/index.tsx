        ...
        {LEAKS.map((l, i) => (
          <Reveal key={l.t} delay={(i % 4) * 60}>
            <div className="grid items-start gap-4 border-b border-border py-7 md:grid-cols-[auto_minmax(0,15rem)_1fr_auto] md:items-center md:gap-8">
              ...
              <span className="text-xs uppercase tracking-[0.14em] text-muted-foreground md:text-right">{l.cost}</span>
            </div>
          </Reveal>
        ))}
      </div>
...
function Agents() {
  ...
  return (
    <Section id="ai-employees">
      ...
      <div className="mt-12">
        <div className="text-xs uppercase tracking-[0.18em] text-muted-foreground">The supporting roles</div>
      ...
      <Reveal>
        <div className="mt-12 rounded-3xl border border-border bg-card p-8 shadow-elegant md:p-10">
          <div className="flex flex-wrap items-baseline justify-between gap-3">
            ...
            <p className="max-w-md text-sm leading-relaxed text-muted-foreground">{featured.summary}</p>
          </div>
          <div className="mt-8">
        ...
        {rest.map((a, i) => (
          <Reveal key={a.slug} delay={(i % 3) * 60}>
            <div className="grid gap-2 border-b border-border py-6 md:grid-cols-[auto_minmax(0,16rem)_1fr] md:items-center md:gap-8">
              <span className="text-xs tracking-[0.16em] text-primary/70">{String(i + 2).padStart(2, "0")}</span>
              <h3 className="text-base font-semibold text-foreground">{a.name}</h3>
              <p className="text-sm leading-relaxed text-muted-foreground">{a.summary}</p>
            </div>
          </Reveal>
            ...
            >
              ...
              <ol className="grid gap-3 sm:grid-cols-2">
                {ind.workflow.map((w, wi) => (
                  <li key={w.label} className="flex gap-3 rounded-xl border border-border bg-background px-4 py-3">
  ...
  return (
    <Section tone="muted" id="process">
      ...
      <div className="relative mt-14">
        <div className="pointer-events-none absolute left-[7px] top-2 bottom-2 w-px bg-primary/20 md:left-0 md:right-0 md:top-[7px] md:bottom-auto md:h-px md:w-auto" aria-hidden="true" />
        ...
        <div className="grid gap-10 lg:grid-cols-[0.85fr_1.15fr] lg:items-center">
          <div>
            ...
            <p className="mt-4 leading-relaxed text-muted-foreground">
              Web design, SEO, and lead generation remain part of what we do — they're the infrastructure that feeds the AI
              systems answering and following up.
            </p>
            <div className="mt-7">
              <SecondaryCta label="Explore websites & growth" to="/growth-services" location="home-growth" />
            </div>
          </div>
        ...
        <div>
          ...
          <dl className="mt-10 space-y-4 border-t border-border pt-8 text-sm">
            ...
            <div className="flex flex-wrap gap-x-3">
              ...
              <dd>
                <a href={`mailto:${CONTACT.email}`} className="font-medium text-foreground hover:text-primary">{CONTACT.email}</a>
              </dd>
            </div>
            <div className="flex flex-wrap gap-x-3">
              <dt className="text-muted-foreground">Prefer to talk?</dt>
              <dd>
                <a href={CONTACT.calendly} target="_blank" rel="noopener noreferrer" className="font-medium text-foreground hover:text-primary">
